...
public class HttpClientUtil {
	...
	public static byte[] doGet(HashMap<String, String> header, String url) throws HttpException, IOException {
		...
		// 设置 HTTP 连接超时为1小时
		httpClient.getHttpConnectionManager().getParams().setConnectionTimeout(3600 * 1000);
		// 2.生成GetMethod对象并设置参数
		GetMethod getMethod = new GetMethod(url);
		// 设置get请求超时为5秒
		getMethod.getParams().setParameter(HttpMethodParams.SO_TIMEOUT, 3600 * 1000);
	...
	public static String doPost(HashMap<String, String> header, String url, String body)
		...
		postMethod.getParams().setParameter(HttpMethodParams.SO_TIMEOUT, 3600 * 1000);
		// 设置请求重试处理，用的是默认的重试处理：请求三次
